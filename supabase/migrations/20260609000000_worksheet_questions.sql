-- Normalize worksheet questions from worksheets.questions JSONB array to worksheet_questions rows.
...
create table if not exists public.worksheet_questions (
  ...
  question_text text not null check (length(question_text) > 0 and length(question_text) <= 4000),
  ...
  constraint worksheet_questions_row_valid check (
...
create index if not exists worksheet_questions_worksheet_id_order_idx
  on public.worksheet_questions (worksheet_id, question_order);

create index if not exists worksheet_questions_worksheet_id_idx
  on public.worksheet_questions (worksheet_id);

alter table public.worksheet_questions enable row level security;

drop policy if exists "Users can read own worksheet questions" on public.worksheet_questions;
create policy "Users can read own worksheet questions"
...
revoke all on table public.worksheet_questions from anon;
grant select on table public.worksheet_questions to authenticated;
...
revoke all on function public._worksheet_question_row_to_jsonb(uuid, integer, text, jsonb, jsonb, jsonb) from public;

-- ---------------------------------------------------------------------------
-- Backfill from worksheets.questions
-- ---------------------------------------------------------------------------
...
select
  ...
  q.value->'given_values',
  q.value->'target_variable',
  q.value->'solution'
from public.worksheets w
cross join lateral jsonb_array_elements(w.questions) as q(value)
where jsonb_array_length(w.questions) > 0
...
declare
  v_mismatch record;
begin
  select w.id, jsonb_array_length(w.questions) as expected_count, count(wq.id) as actual_count
  into v_mismatch
  from public.worksheets w
  left join public.worksheet_questions wq on wq.worksheet_id = w.id
  group by w.id, w.questions
  having jsonb_array_length(w.questions) <> count(wq.id)
  limit 1;

  if found then
    raise exception 'Backfill count mismatch for worksheet % (expected %, got %)',
      v_mismatch.id, v_mismatch.expected_count, v_mismatch.actual_count;
  end if;
end;
$$;

-- ---------------------------------------------------------------------------
-- saved_question_count: trigger-maintained instead of generated from JSONB
-- ---------------------------------------------------------------------------

alter table public.worksheets
  drop column if exists saved_question_count;

alter table public.worksheets
  add column saved_question_count integer not null default 0;

update public.worksheets w
set saved_question_count = (
...
drop trigger if exists worksheet_questions_saved_count on public.worksheet_questions;
create trigger worksheet_questions_saved_count
  after insert or delete on public.worksheet_questions
  for each row
  execute function public._sync_worksheet_saved_question_count();

-- ---------------------------------------------------------------------------
-- Idempotency reconcile helpers
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Drop legacy worksheets.questions JSONB column (after backfill)
-- ---------------------------------------------------------------------------
...
drop function if exists public.is_valid_worksheet_questions_array(jsonb, integer);

-- ---------------------------------------------------------------------------
-- Worksheet init / append / replace / regenerate (non-idempotency paths)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Credit reservation + idempotency RPCs (question row storage)
-- ---------------------------------------------------------------------------
