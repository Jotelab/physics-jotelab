-- Restrict worksheets to physics only.

delete from public.worksheets where subject <> 'physics';

alter table public.worksheets
  drop constraint if exists worksheets_subject_check,
  add constraint worksheets_subject_check check (subject = 'physics');
...
begin
  ...
  if p_subject is null or p_subject <> 'physics' then
