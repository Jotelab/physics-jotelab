-- A builder "Given" selection constrains which quantity must appear, but most
-- presets intentionally leave its numeric value for the generator to choose.
-- Preserve the strict question-value validator and relax only generation
-- settings by accepting the target-variable shape when `value` is omitted.
...
revoke all on function public.is_valid_generation_settings_given_constraint(jsonb) from public;
revoke all on function public.is_valid_generation_settings_given_constraint(jsonb) from anon;
revoke all on function public.is_valid_generation_settings_given_constraint(jsonb) from authenticated;
