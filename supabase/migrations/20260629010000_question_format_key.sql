-- Allow an optional `format` discriminant on worksheet question JSON.
--
-- Question formats are modeled as a discriminated union in the app
-- (features/generate/schemas.ts). The calculation format is stored by its
-- columns (question_text/given_values/target_variable/solution), so `format`
-- is not persisted — but the question JSON handed to the save/replace RPCs now
-- carries it. Permit the key (validated against the allowlist) without
-- requiring it, so both legacy rows (no key) and new payloads validate.
--
-- `is_valid_worksheet_questions_array` delegates to this function, so updating
-- it here covers the array validator too.
